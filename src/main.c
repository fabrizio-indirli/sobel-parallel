...
#if LOGGING
    #define LOG_FILENAME "./logs_plots/plog_mpi_n4_N1.csv"
    FILE *fOut;
    ...
    if(my_rank == 0){
        ...
        apply_all_filters0(image->width, image->height, p, n_imgs_per_node[0]);
            ...
            for(i=1; i < num_nodes; i++){
                for(j=0; j < n_imgs_per_node[i]; j++){
                    ...
                    MPI_Recv(p[n_prev_imgs], W0*H0, mpi_pixel_type, i,3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    n_prev_imgs++;
                }
            }
        #endif
        ...
        // NODES WITH RANK >= 1
                ...
                if(n_imgs_this_node > 0){
                    ...
                    apply_all_filters(dims, &(dims[n_imgs_this_node]), p_rec, n_imgs_this_node, mpi_pixel_type);
