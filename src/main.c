...
apply_blur_filter( animated_gif * image, int size, int threshold )
{
    ...
    if (image->n_images < num_threads) {
        ...
        /* Process all images */
        #pragma omp parallel default(none) private(i,j,k,width,height,new,n_iter,end) shared(p,size,image,threshold)
        {
            #pragma omp for schedule(static)
            for ( i = 0 ; i < image->n_images ; i++ )
            {
                int rank = omp_get_thread_num();
                printf("(BLUR-ELSE)Image[%d] from %d\n", i, rank);
...
apply_sobel_filter( animated_gif * image )
{
    ...
    if (image->n_images < num_threads) {
        ...
        #pragma omp parallel default(none) private(i,j,k,width,height) shared(p,image)
        {
            #pragma omp for schedule(static)
            for ( i = 0 ; i < image->n_images ; i++ )
            {
                int rank = omp_get_thread_num();
                printf("(SOBEL-ELSE)Image[%d] from %d\n", i, rank);
...
int main( int argc, char ** argv )
{
    ...
    duration = (t2.tv_sec -t1.tv_sec)+((t2.tv_usec-t1.tv_usec)/1e6);
    double durFilters = (t2.tv_sec -t0.tv_sec)+((t2.tv_usec-t0.tv_usec)/1e6);
    printf( "SOBEL done in %lf s\n", duration ) ;
    printf( "SO FAR done in %lf s\n",  durFilters) ;
    // fprintf(fptr,"\t %f", duration);
    fprintf(fptr,", \t %f", duration);
    fprintf(fptr,", \t %f", durFilters);
